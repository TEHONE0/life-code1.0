...
HEALTH_LEVEL:[必须严格执行上文「## 二」中的「健康层级判定」全套规则得出，1-9整数。强制：以 Q08 防御反应灵活度为主锚点、结论以主权重型号的该层级为准、Lv.2–5 是正常人区间、Lv.7–9 仅当出现明确伤害自己/他人信号才给、拿不准往健康一侧给。此值必须与完整报告的判定结果一致，不得另起一套标准]
WEIGHT:[W主权重型名，如"独特型"]
